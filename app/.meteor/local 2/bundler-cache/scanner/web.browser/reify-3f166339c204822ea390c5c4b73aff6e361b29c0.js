module.export({SimpleSchema:()=>SimpleSchema,ValidationContext:()=>ValidationContext});module.export({schemaDefinitionOptions:()=>schemaDefinitionOptions},true);let MongoObject;module.link('mongo-object',{default(v){MongoObject=v}},0);let clean;module.link('./clean.js',{default(v){clean=v}},1);let getDefaultErrorMessage;module.link('./defaultMessages.js',{getDefaultErrorMessage(v){getDefaultErrorMessage=v}},2);let ClientError;module.link('./errors.js',{ClientError(v){ClientError=v}},3);let expandShorthand;module.link('./expandShorthand.js',{default(v){expandShorthand=v}},4);let SimpleSchemaGroup;module.link('./SimpleSchemaGroup.js',{default(v){SimpleSchemaGroup=v}},5);let forEachKeyAncestor,humanize,isEmptyObject;module.link('./utility/index.js',{forEachKeyAncestor(v){forEachKeyAncestor=v},humanize(v){humanize=v},isEmptyObject(v){isEmptyObject=v}},6);let ValidationContext;module.link('./ValidationContext.js',{default(v){ValidationContext=v}},7);var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
...
class SimpleSchema {
    ...
    */
    ...
    keyIsInBlackBox(key) {
        ...
        forEachKeyAncestor(genericKey, (ancestor, remainder) => {
...
function inflectedLabel(fieldName, shouldHumanize = false) {
    ...
    return (label != null && shouldHumanize) ? humanize(label) : (label !== null && label !== void 0 ? label : '');
}
