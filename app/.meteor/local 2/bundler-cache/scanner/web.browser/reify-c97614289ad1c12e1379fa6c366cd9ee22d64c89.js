let clone;module.link('clone',{default(v){clone=v}},0);let MongoObject;module.link('mongo-object',{default(v){MongoObject=v}},1);let convertToProperType;module.link('./clean/convertToProperType.js',{default(v){convertToProperType=v}},2);let setAutoValues;module.link('./clean/setAutoValues.js',{default(v){setAutoValues=v}},3);let SimpleSchema;module.link('./SimpleSchema.js',{SimpleSchema(v){SimpleSchema=v}},4);let isEmptyObject,looksLikeModifier;module.link('./utility/index.js',{isEmptyObject(v){isEmptyObject=v},looksLikeModifier(v){looksLikeModifier=v}},5);let isValueTypeValid;module.link('./validation/typeValidator/index.js',{isValueTypeValid(v){isValueTypeValid=v}},6);
...
function clean(ss, doc, options = {}) {
    ...
    const mongoObject = (_a = cleanOptions.mongoObject) !== null && _a !== void 0 ? _a : new MongoObject(cleanDoc, ss.blackboxKeys());
    // Clean loop
    ...
    if (cleanOptions.getAutoValues === true) {
        setAutoValues(ss.autoValueFunctions(), mongoObject, cleanOptions.isModifier || false, cleanOptions.isUpsert || false, cleanOptions.extendAutoValueContext);
    }
